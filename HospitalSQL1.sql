create database manoj;
use manoj;

#STEP 1: Identify Main Entities (Tables)
#From the ER diagram, the main entities are:
#1.Patient
#2.Doctor
#3.Employee (Parent)
#4.Nurse (Child of Employee)
#5.Receptionist (Child of Employee)
#6.Rooms
#7.Bills
#8.Test_Report
#9.Records
CREATE TABLE Patient (
    P_ID INT PRIMARY KEY,
    Name VARCHAR(50),
    Gender VARCHAR(10),
    Age INT,
    DOB DATE,
    Mob_No VARCHAR(15)
);
CREATE TABLE Doctor (
    D_ID INT PRIMARY KEY,
    Name VARCHAR(50),
    Dept VARCHAR(50),
    Qualification VARCHAR(100),
    Mob_No VARCHAR(15)
);
CREATE TABLE Employee (
    E_ID INT PRIMARY KEY,
    Name VARCHAR(50),
    Salary DECIMAL(10,2),
    Sex VARCHAR(10),
    Mob_No VARCHAR(15),
    State VARCHAR(50),
    City VARCHAR(50),
    Pin_No VARCHAR(10)
);
CREATE TABLE Nurse (
    E_ID INT PRIMARY KEY,
    FOREIGN KEY (E_ID) REFERENCES Employee(E_ID)
);
CREATE TABLE Receptionist (
    E_ID INT PRIMARY KEY,
    FOREIGN KEY (E_ID) REFERENCES Employee(E_ID)
);
CREATE TABLE Rooms (
    R_ID INT PRIMARY KEY,
    Type VARCHAR(50),
    Capacity INT,
    Availability VARCHAR(20)
);
CREATE TABLE Bills (
    B_ID INT PRIMARY KEY,
    Amount DECIMAL(10,2),
    P_ID INT,
    FOREIGN KEY (P_ID) REFERENCES Patient(P_ID)
);
CREATE TABLE Test_Report (
    R_ID INT,
    P_ID INT,
    Test_Type VARCHAR(50),
    Result VARCHAR(100),
    PRIMARY KEY (R_ID, P_ID),
    FOREIGN KEY (R_ID) REFERENCES Rooms(R_ID),
    FOREIGN KEY (P_ID) REFERENCES Patient(P_ID)
);
CREATE TABLE Records (
    Record_No INT PRIMARY KEY,
    App_No INT
);


#STEP 3: Relationship Tables (M:N)

CREATE TABLE Consults (
    P_ID INT,
    D_ID INT,
    PRIMARY KEY (P_ID, D_ID),
    FOREIGN KEY (P_ID) REFERENCES Patient(P_ID),
    FOREIGN KEY (D_ID) REFERENCES Doctor(D_ID)
);
CREATE TABLE Assigned (
    P_ID INT,
    R_ID INT,
    PRIMARY KEY (P_ID, R_ID),
    FOREIGN KEY (P_ID) REFERENCES Patient(P_ID),
    FOREIGN KEY (R_ID) REFERENCES Rooms(R_ID)
);
CREATE TABLE Governs (
    E_ID INT,
    R_ID INT,
    PRIMARY KEY (E_ID, R_ID),
    FOREIGN KEY (E_ID) REFERENCES Nurse(E_ID),
    FOREIGN KEY (R_ID) REFERENCES Rooms(R_ID)
);
CREATE TABLE Maintains (
    E_ID INT,
    Record_No INT,
    PRIMARY KEY (E_ID, Record_No),
    FOREIGN KEY (E_ID) REFERENCES Receptionist(E_ID),
    FOREIGN KEY (Record_No) REFERENCES Records(Record_No)
);

















